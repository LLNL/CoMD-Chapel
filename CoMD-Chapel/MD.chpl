...
local {
      forall box in MyDom.cells[MyDom.localDom] with (+ reduce vcmTemp) {
        for atom in box.atoms[1..box.count] {
          vcmTemp += atom.v;
        }
      }
...
local {
      ...
      forall (box, pe) in zip(MyDom.cells[MyDom.localDom], MyDom.pe) with (+ reduce keTemp, + reduce peTemp) {
        //Why is this sooo slow as measured on grid 100x100x100
        //is it zippered iterations or array slices
        /*for (atom, pot) in zip(box.atoms[1..box.count], pe[1..box.count]) {
...
///*
          ...
          for i in 2..box.count {
            const xi = box.atoms[i];
            var j = i-1;
            while (j >= 1 && box.atoms[j] > xi) {
...
inline proc gatherAtoms(const ref MyDom:Domain, const in face : int) : int(32) {
  local {
    ...
    forall (dBox, sBox, dBoxIdx) in zip(MyDom.cells[dest[face]], faceArr, faceArr.domain) with (+ reduce numLocalAtoms) {
      ...
      dBox.count += sBox.count;
      // assert(dBox.count <= MAXATOMS);
      for sAtom in sBox.atoms[1..sBox.count] {
        count += 1;
        dBox.atoms[count] = sAtom;
        dBox.atoms[count].r += MyDom.pbc[face];
      }
      if(MyDom.localDom.member(dBoxIdx)) then numLocalAtoms += sBox.count; 
    }
...
/*
  ...
  on locGrid[nf] {
    ...
    local {
      ...
      // why not put const ref g2
      coforall (g1, g2) in zip(gTmp, g.cells[sf]) with (ref g1, ref g2) {
        g1.count = g2.count;
        //g1.atoms[1..g1.count] = g2.atoms[1..g2.count];
        for i in 1..g1.count do { g1.atoms[i] = g2.atoms[i]; }
        //for (g1a, g2a) in zip(g1.atoms[1..g1.count], g2.atoms[1..g2.count]) do g1a = g2a;
      }
    ...
    local {
      ...
      //if(face % 2) then g.nP$.writeEF(true); else g.nM$.writeEF(true);
    }
...
*/
  if useChplVis then const g0 = Grid[(0,0,0)];
  // This works but is slow 
  //coforall (dArr, sArr) in zip(faceArr, gTmp) with (ref dArr, ref sArr) {
  coforall (dArr, sArr) in zip(faceArr, Grid[nf].cells[src]) with (ref dArr, const ref sArr) {
    dArr.count = sArr.count;
    //dArr.atoms[1..dArr.count] = sArr.atoms[1..sArr.count];
    for i in 1..dArr.count do { dArr.atoms[i] = sArr.atoms[i]; }
    //for (da, sa) in zip(dArr.atoms[1..dArr.count], sArr.atoms[1..sArr.count]) do da = sa;
  }
  if(face % 2) then Grid[nf].nP$.writeXF(true); else Grid[nf].nM$.writeXF(true);
  if useChplVis then const g0 = Grid[(0,0,0)]; // chplvis marker
}
	    ...
	    for iy in start(2)..end(2) {
	      for iz in start(3)..end(3) {
            for ib in 1..nb {
              ...
              MyDom.cells[box].atoms[MyDom.cells[box].count] = new Atom(gid=gid, mass=mass, species=1, name=name, r=(rx, ry, rz), v=(0.0,0.0,0.0));
...
local {
      forall (box, f) in zip(MyDom.cells[MyDom.localDom], MyDom.f) {
        //Why is this sooo slow as measured on grid 100x100x100
        //is it zippered iterations or array slices
        /*for (a, fa) in zip(box.atoms[1..box.count], f[1..box.count]) {
          a.v += ((dt/a.mass)*fa);
        }*/
        for i in 1..box.count {
          ref a = box.atoms[i];
...
local {
      forall box in MyDom.cells[MyDom.localDom] {
        //Why is this sooo slow as measured on grid 100x100x100
        //is it zippered iterations or array slices
        /*for a in box.atoms[1..box.count] {
          a.r += (dt*a.v);
        }*/
        for i in 1..box.count {
         ref a = box.atoms[i];
